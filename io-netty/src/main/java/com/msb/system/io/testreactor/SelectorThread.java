...
/*
每个SelectorThread对应一个Selector线性处理
*/
public class SelectorThread extends ThreadLocal<LinkedBlockingQueue<Channel>> implements Runnable {
    ...
    /*
    重写initialValue方法, 返回线程的阻塞队列
    get() ---> return setInitialValue() ---> T value = initialValue(); return value;
     */
    @Override
...
//                System.out.println(Thread.currentThread().getName()+"   :  after select...." + selector.keys().size());
    ...
    private void acceptHandler(SelectionKey key) {
        ...
        try {
            //接收到客户端的连接后, 再进行selector的分配, 并把client的socket连接fd, 注册到selector上
            SocketChannel client = server.accept();
            client.configureBlocking(false);

            /*
            如果在是单线程环境下,就直接register;
            但是在多线程环境下, 需要多selector工作的环境下, 需要选择一个selector去注册
             */
