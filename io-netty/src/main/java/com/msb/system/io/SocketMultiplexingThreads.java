...
public class SocketMultiplexingThreads {
  ...
  public static void main(String[] args) {
    ...
    // T1线程(Boss)中维护容量为2一个BlockingQueue队列
  ...
  public static class NioThread extends Thread {
    ...
    static final AtomicInteger idx = new AtomicInteger();

    NioThread(Selector sel, int n) {
      this.selector = sel;
      this.selectorCount = n;

      queue = new LinkedBlockingQueue[selectorCount];
      for (int i = 0; i < n; i++) {
    ...
    NioThread(Selector sel) {
      this.selector = sel;
      id = idx.getAndIncrement() % selectorCount;
    ...
    public void acceptHandler(SelectionKey key) {
      try {
        ...
        int num = idx.getAndIncrement() % selectorCount;
